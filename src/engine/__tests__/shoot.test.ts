...
describe('calcProbability', () => {
  ...
  it('ポジション修正が加算される', () => {
    // (2-2+3)×10 + 15(DF修正) = 30+15 = 45
    expect(calcProbability(2, 2, 10, 15)).toBe(45);
  });

  it('ZOC修正が加算される', () => {
    // (2-2+3)×10 + 0 + (-5×2) = 30 - 10 = 20
    expect(calcProbability(2, 2, 10, 0, -10)).toBe(20);
  });

  it('0%にクランプされる（コスト差-2の限界）', () => {
    // (1-3+3)×10 - 20 = 10 - 20 = -10 → 0
    expect(calcProbability(1, 3, 10, -20)).toBe(0);
  });

  it('100%にクランプされる', () => {
    // 3×18 + 30 = 54+30 = 84→ not clamped; add more
