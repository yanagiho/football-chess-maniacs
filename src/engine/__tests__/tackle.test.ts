...
// ============================================================
describe('タックル確率表 (Ω=18, ポジション修正なし)', () => {
  // 守備コスト × 攻撃コスト × 期待確率
  const TABLE: [Cost, Cost, number][] = [
    [1, 1, 54],   [1, 1.5, 45], [1, 2, 36],   [1, 2.5, 27], [1, 3, 18],
    [1.5, 1, 63], [1.5, 1.5, 54], [1.5, 2, 45], [1.5, 2.5, 36], [1.5, 3, 27],
    [2, 1, 72],   [2, 1.5, 63], [2, 2, 54],   [2, 2.5, 45], [2, 3, 36],
    [2.5, 1, 81], [2.5, 1.5, 72], [2.5, 2, 63], [2.5, 2.5, 54], [2.5, 3, 45],
    [3, 1, 90],   [3, 1.5, 81], [3, 2, 72],   [3, 2.5, 63], [3, 3, 54],
  ];
...
describe('タックル: ポジション修正', () => {
  it.each([
    ...
    ['SB', 2, 'WG', 2, 54],
    // 仕様書注記: DF(cost1,+20) vs WG(cost3,-10): (1-3+3)×18+20-10 = 18+20-10 = 28
    ['DF', 1, 'WG', 3, 28],
  ] as const)(
